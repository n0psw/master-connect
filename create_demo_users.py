...
async def create_demo_users():
    ...
    async with async_session() as session:
        ...
        if existing_student:
            print(f"⚠️  Студент {student_email} уже существует, обновляем пароль...")
            existing_student.password_hash = get_password_hash(demo_password)
        else:
            student_user = User(
                email=student_email,
        ...
        if existing_mentor_user:
            print(f"⚠️  Ментор {mentor_email} уже существует, обновляем пароль...")
            existing_mentor_user.password_hash = get_password_hash(demo_password)
        else:
            mentor_user = User(
                email=mentor_email,
                password_hash=get_password_hash(demo_password),
        ...
        if existing_admin:
            print(f"⚠️  Админ {admin_email} уже существует, обновляем пароль...")
            existing_admin.password_hash = get_password_hash(demo_password)
        else:
            admin_user = User(
                email=admin_email,
                password_hash=get_password_hash(demo_password),
